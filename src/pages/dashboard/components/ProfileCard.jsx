...
const ProfileCard = () => {
  return (
    <div className="bg-white rounded-xl shadow p-6 flex flex-col items-center w-full gap-4">
      ...
      <h2 className="text-lg font-semibold">Reggie Coaster</h2>
      <p className="text-sm text-gray-500">Senior Software Engineer</p>
      <p className="text-xs text-gray-400 mb-2">Islamabad, Pakistan</p>
      ...
      <div className="w-full">
        ...
        <span className="text-lg text-gray-600">Top Skills</span>
        <button className='hover:bg-gray-100 rounded-full p-2'>
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          {['Enterprise Java', 'API Development', 'Python', 'Cloud Computing'].map(skill => (
            <span key={skill} className="bg-green-100 text-green-700 px-2 py-1 rounded-full text-xs font-medium">{skill}</span>
          ))}
        </div>
      </div>
      </div>
